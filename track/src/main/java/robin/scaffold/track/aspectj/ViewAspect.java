...
public class ViewAspect {
    ...
    public void afterRecycleAdapterOnBind(JoinPoint joinPoint) {
        ...
        try {
            // 获取listView对象
            Field recyclerViewField = ReflectUtils.INSTANCE.getDeclaredField(viewHolder, "mOwnerRecyclerView");
            recyclerViewField.setAccessible(true);
            RecyclerView recyclerView = (RecyclerView) recyclerViewField.get(viewHolder);
            // 获取listView对象
            Tracker.INSTANCE.setViewTracker(viewHolder.itemView, Tracker.INSTANCE.getRecyclerViewPath(viewHolder.itemView, recyclerView, position));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
